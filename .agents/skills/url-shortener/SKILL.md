...
```json
{
  ...
  "shortLink": "https://link.brokertricks.com/custom-slug"
}
```
...
```bash
curl -X POST https://link.brokertricks.com/api/link/create \
...
```bash
curl https://link.brokertricks.com/api/link/list \
  -H "Authorization: Bearer YOUR_TOKEN"
```
...
```bash
curl -X POST https://link.brokertricks.com/api/link/delete \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"slug": "my-slug"}'
```
